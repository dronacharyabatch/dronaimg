...
var IMG_URL = "http://10.10.41.149:8080/view?";
var TEMPLATE = "{0} :\n{1}";
...
function formatLink(mediaUrl, owner){
	return IMG_URL+"MediaUrl="+mediaUrl+"&owner="+owner;
}
function processGroup(reqData, callback){
	var group = [];
	var sender;
	executeQuery('select dt.name, dt.mobile, dt.role, dr.auth_key, dr.auth_secret from dronateam dt, dronaroom dr where dt.is_block = 0 and dt.room = dr.room and dr.is_expired = 0', (status, result)=>{
		...
		if (MediaContentType0 != null && MediaContentType0.trim().length > 0) {
			...
			while(reqData["MediaUrl"+idx] !== undefined){
				var medialLink = reqData["MediaUrl"+idx];
				medialLink = formatLink(medialLink, sender.name);
				mediaUris.push({"MediaUrl" : medialLink, "MediaContentType" : reqData["MediaContentType"+idx]});
				idx++;
			}
		}
